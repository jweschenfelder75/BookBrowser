...
class BookParser:
    ...
    def get_file_title_author_from_book(self, book_id: int, filepath: str, /) -> Book:
        """
        Fetches the title and author (sometimes translator) from a book and assigns some given id to the book.

        Args:
            book_id (int): id that you want to assign to the book
            filepath (str): file path of the book

        Returns:
            Book: book (book_id, book filepath, book title, book author)
        """
        title = None
        author = None
        try:
    ...
    def get_basic_book_stats(self, content: str, line_count: int, /) -> BookStatistics:
        ...
        Returns:
            BookStatistics: book statistics (book line count, book space count, book word count)
        """
    ...
    def get_book_info(self, book: Book, /) -> Book:
        ...
        Args:
            book (Book): book (book id, book filepath, book title, book author)

        Returns:
            Book: book with statistics (book id, book filepath, book title, book author, book line count,
                                        book space count, book word count)
        """
    ...
    def get_book_info_with_regex(self, book: Book, regex: str, /) -> Book:
        ...
        Returns:
            Book: book with statistics (book id, book filepath, book title, book author, book line count,
                                        book space count, book word count, book pattern count)
        """
        content, line_count = self.read_file_and_count_lines(book.file)
        stats = self.get_basic_book_stats(content, line_count)
    ...
    def get_books(self) -> "Bookshelf":
        ...
        Returns:
            Bookshelf: books (Book) as a list
        """
        result = Bookshelf(0)
        book_id = 0
